    ...
    return (
      <div className="min-h-screen bg-[#FFF9FB] flex items-center justify-center px-4">
        <div className="max-w-md text-center bg-white rounded-2xl border border-pink-100 p-8">
          ...
          <h1 className="text-lg font-semibold text-gray-800">Este link ya no está disponible</h1>
    ...
    <div className="min-h-screen bg-[#FFF9FB] py-10 px-4">
      <div className="max-w-3xl mx-auto">
        <header className="text-center mb-8">
          <img src="/logo.svg" alt="Cíclica" className="h-14 w-auto mx-auto mb-4" />
        ...
        <section className="bg-white rounded-2xl border border-pink-100 p-6 mb-4">
          <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-400 mb-4">Resumen</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <div className="text-[10px] uppercase tracking-wider text-gray-400">Ciclo promedio</div>
              <div className="text-2xl font-bold text-gray-800 tabular-nums mt-1">{cycleLength} <span className="text-sm font-normal text-gray-400">días</span></div>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-wider text-gray-400">Variabilidad</div>
              <div className="text-2xl font-bold text-gray-800 tabular-nums mt-1">
                {variability ?? '—'} <span className="text-sm font-normal text-gray-400">días</span>
              </div>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-wider text-gray-400">Ciclos registrados</div>
              <div className="text-2xl font-bold text-gray-800 tabular-nums mt-1">{pastCycles.length}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-wider text-gray-400">Anticonceptivo oral</div>
        ...
        <section className="bg-white rounded-2xl border border-pink-100 p-6 mb-4">
          <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-400 mb-4">Síntomas más frecuentes</h2>
          {topSintomas.length === 0 ? (
            <p className="text-sm text-gray-400">Sin síntomas registrados.</p>
          ) : (
              ...
              <tbody>
                {topSintomas.map(([s, count]) => (
                  <tr key={s} className="border-t border-gray-100">
                    ...
                    <td className="py-2 text-right tabular-nums text-gray-600">{count}×</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="bg-white rounded-2xl border border-pink-100 p-6 mb-4">
          <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-400 mb-4">Historial de ciclos</h2>
          {ciclos.length === 0 ? (
            <p className="text-sm text-gray-400">Sin ciclos registrados.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-400 text-xs">
              ...
              <tbody>
                {ciclos.slice(0, 12).map((c, i) => (
                  <tr key={i} className="border-t border-gray-100">
                    ...
                    <td className="py-2 text-right tabular-nums text-gray-600">{c.duracion_dias} días</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="bg-white rounded-2xl border border-pink-100 p-6 mb-4">
          <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-400 mb-4">Vida sexual</h2>
        ...
        <footer className="text-center text-xs text-gray-400 mt-8 leading-relaxed">
