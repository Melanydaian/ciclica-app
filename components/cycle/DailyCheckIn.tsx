...
export default function DailyCheckIn({ yaRegistroHoy = false }: { yaRegistroHoy?: boolean }) {
  ...
  if (done) {
    return (
      <div className="bg-white dark:bg-[#1F1822] rounded-3xl border border-pink-100 dark:border-[#3A2F3F] px-5 py-5">
        <div className="flex items-center gap-3">
          <span className="text-2xl">✅</span>
          <div>
            <p className="text-sm font-semibold text-gray-800 dark:text-[#F4F1F5]">Ya registraste hoy</p>
            <p className="text-xs text-gray-500 dark:text-[#B4ABB8] mt-0.5">+5 pts · Tu racha sigue 🔥</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="bg-white dark:bg-[#1F1822] rounded-3xl border border-pink-100 dark:border-[#3A2F3F] px-5 py-5">
      <div className="flex items-baseline justify-between mb-4">
        <div>
          ...
          <p className="text-base font-semibold text-gray-800 dark:text-[#F4F1F5] mt-1">¿Cómo te sentís hoy?</p>
        </div>
        {mood && (
          <button
            type="button"
        ...
        {MOODS.map(m => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[9px] mt-1 font-medium ${active ? 'text-pink-600' : 'text-gray-400 dark:text-[#8A8190]'}`}>
                {m.label}
              </span>
            </button>
          )
      ...
      {mood && (
        <div className="mt-4 animate-in fade-in slide-in-from-top-1 duration-200">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-400 dark:text-[#8A8190] mb-2">
            ¿Algún síntoma? <span className="text-gray-400 dark:text-[#8A8190] normal-case tracking-normal">(opcional)</span>
          </p>
