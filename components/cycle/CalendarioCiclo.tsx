    ...
    <div className="bg-white dark:bg-[#1F1822] rounded-2xl border border-pink-100 dark:border-[#3A2F3F] px-5 py-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <div className="text-base font-semibold text-gray-800 dark:text-[#F4F1F5] capitalize mt-1">{monthName}</div>
        </div>
      ...
      <div className="grid grid-cols-7 gap-1 mb-2">
        {['D', 'L', 'M', 'M', 'J', 'V', 'S'].map((d, i) => (
          <div key={i} className="text-center text-[10px] font-bold text-gray-400 dark:text-[#8A8190] tracking-wider py-1">
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="mt-5 pt-4 border-t border-gray-100 dark:border-[#3A2F3F] space-y-2">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
          {Object.entries(PHASE_DOT).map(([key, { bg, ring, label }]) => (
            <div key={key} className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-gray-600 dark:text-[#C9BFCB]">{label}</span>
            </div>
          ))}
        </div>
        <div className="flex items-center gap-2 mt-2">
          <span className="w-3 h-3 rounded-md border-2 border-dashed border-pink-500" />
          <span className="text-[11px] text-gray-600 dark:text-[#C9BFCB]">Próximo período aproximado · {fmtRange(rangeStart, rangeEnd)}</span>
        </div>
      </div>

      <Link
        href="/dashboard/historial"
        className="mt-5 -mx-5 -mb-6 px-5 py-3.5 border-t border-pink-100 flex items-center justify-between text-pink-600 hover:bg-pink-50 dark:bg-pink-500/10 transition-colors text-sm font-semibold"
      >
